...
export default class viewLaboursList extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      labList: [],
  ...
  componentDidMount() {
    ...
    fetch(`${host}/users/viewLaboursList`, {
      method: 'POST',
      headers: {
        Authorization: token,
      ...
      if (json.success) {
        this.setState({ labList: json.users });
  ...
  render() {
    const { route, navigation } = { ...this.props };
    const{ user, userType, token } = route.params;
    const { labList, error } = { ...this.state };
    return (
      <ScreenContainer style={{
        ...
        <ScrollView>
        {!error && labList.length > 0 && labList.map((val) => {
          const { username, user_email, mobileNum, details } = val;
          return (
                ...
                <TouchableOpacity
                  onPress={() => {
                    navigation.push('ViewMe', {
                      user: user_email, userType: 'labour', token, details: 1, userDetails: null, skillList: null,
                    });
                  }}
                  styles={{ ...styles.button, flex: 1 }}
                >
                  ...
                  onPress={() => {
                    Alert.alert(
                      ...
                      [
                        {
                          text: "Yes",
                          onPress: () => this.removeLabour(user_email),
                        },
                        {
                          text: "No",
                          onPress: () => {},
