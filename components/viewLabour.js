...
import {
  Text, TextInput, View, Button, SafeAreaView, ScrollView,
} from 'react-native';
...
export default class ViewMe extends React.Component {
  ...
  viewDetails() {
    const { route, navigation } = { ...this.props };
    const {
      user, userType, details, token, userDetails, skillList, username,
    } = route.params;
    const { signOut } = this.context;
    fetch(`${host}/users/viewLabour`, {
      ...
      if (json.success) {
        if (json.msg === 'Details not entered') {
          ...
          if (userDetails) {
            this.setState({ userDetails, skillList, detailsPresent: true });
          }
          this.setState({
            village: json.users[0].village, city: json.users[0].city, state: json.users[0].state, skills, detailsPresent: true, username: json.users[0].username,
          });
        }
  ...
  editMyDetails() {
    ...
    const { village, city, state, skills, username } = { ...this.state };
    this.setState({ userDetails: [village, city, state], skillList: skills.join(':'), detailsPresent: false });
    navigation.navigate('ViewMe', {
      user, userType, token, skillList: skills.join(':'), userDetails:[village, city, state, username], details: 0,
    });
  }
  ...
  applicationHandler() {
    ...
    const {
      skills, village, city, state, userDetails, skillList, username
    } = { ...this.state };
  ...
  render() {
    const { route, navigation } = { ...this.props };
    const {
      user, userType, token, skillList, userDetails, details,
    } = route.params;
    const {
      village, city, state, skills, skillNum, error, detailsPresent, username,
    } = { ...this.state };
    ...
    if (userDetails && skillList && this.updateDetails) {
      ...
      this.setState({
        village: userDetails[0], city: userDetails[1], state: userDetails[2], skillNum: skillNumDup, skills: skillSet, username: userDetails[3],
      });
    }
    return (
